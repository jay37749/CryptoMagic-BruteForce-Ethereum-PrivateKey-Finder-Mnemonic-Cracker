...
def Main():
    ...
    parser = argparse.ArgumentParser(description="Polkadot Address Finder Script")
    
    parser.add_argument('-f', '--file', dest="filenameDot", required=True,
                        help="Polkadot Rich Address File With Type Format .TXT [Example: -f dot1000.txt]")
    parser.add_argument('-v', '--view', dest="ViewPrint", default='10000', type=int,
                        help="Print After Generated This Number Print And Report (Default: 10000)")
    parser.add_argument('-n', '--thread', dest="ThreadCount", default='8', type=int,
                        help="Total Thread Number (Total Core CPU) (Default: 8)")

    # Parse arguments
    args = parser.parse_args()
    filename = args.filenameDot
